...
if($num>0){
  ...
  while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    array_push($employee_arr['register'],$employee_item['Gender']);
  }
  echo "Distribuição de gêneros M (masculino) e F (feminino):
    ";
  echo json_encode(array_count_values($employee_arr['register']));
} else{
  echo json_encode(
